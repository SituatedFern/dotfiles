...
const vimrc_1 = require("./src/configuration/vimrc");
...
async function activate(context, handleLocal = true) {
    ...
    registerEventListener(context, vscode.workspace.onDidSaveTextDocument, async (document) => {
        if (configuration_1.configuration.vimrc.enable &&
            vimrc_1.vimrc.vimrcPath &&
            path.relative(document.fileName, vimrc_1.vimrc.vimrcPath) === '') {
    ...
    registerCommand(context, 'vim.editVimrc', async () => {
        if (vimrc_1.vimrc.vimrcPath) {
            const document = await vscode.workspace.openTextDocument(vimrc_1.vimrc.vimrcPath);
